...
# double recomp  - parallelize in H no matter the cost
...
def linear_xent_bwd_kernel_matmul_t_dA(
    ...
    for idx_N in range(N // N_BLOCK_SIZE):
        ...
        z_j_to_k = tl.zeros((N_BLOCK_SIZE, V_BLOCK_SIZE), dtype=tl.float32)
        for _ in range(H // H_BLOCK_SIZE):
            x_chunk = tl.load(x_fwd_block_ptr)  # Nc x Hc
            A_v = tl.load(A_fwd_block_ptr)  # Hc x Vc
        ...
        x_chunk_bwd = tl.load(x_bwd_block_ptr)  # Hc x Nc
        # Agrad
...
def linear_xent_bwd_kernel_matmul_t_dx(
    ...
    for idx_V in range(V // V_BLOCK_SIZE):
        ...
        for idx_H_1 in range(H // H_BLOCK_SIZE):
            ...
            A_v_fwd = tl.load(A_fwd_block_ptr)  # Hc x Vc

            z_j_to_k = tl.dot(x_chunk, A_v_fwd, z_j_to_k)  # (Nc x Hc) @ (Hc x Vc)
        ...
        A_v = tl.load(A_bwd_block_ptr).trans()  # Hc x Vc
        # xgrad
