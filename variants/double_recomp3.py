...
def linear_xent_bwd_kernel_matmul_t_dA(
    ...
    # block = tl.arange(0, H_BLOCK_SIZE)[:, None] * tl.arange(0, V_BLOCK_SIZE)[None, :]
    # A_grad_block_ptr = A_grad_ptr + stride_A_H * 0 + stride_A_V * idx_V + block
    ...
    for idx_H in range(H // H_BLOCK_SIZE):
        ...
        # tl.atomic_add(A_grad_block_ptr, temp_AgradT.to(tl.float16))
...
def linear_xent_bwd_kernel_matmul_t_dx(
    ...
    #     order=(1, 0),
    # )
    # block = tl.arange(0, N_BLOCK_SIZE)[:, None] * tl.arange(0, H_BLOCK_SIZE)[None, :]
    # x_grad_block_ptr = x_grad_ptr + stride_x_N * idx_N + stride_x_H * 0 + block
    ...
    for idx_H in range(H // H_BLOCK_SIZE):
        ...
        # x_grad_block_ptr += H_BLOCK_SIZE

        # x_grad_block_ptr = tl.advance(x_grad_block_ptr, [0, H_BLOCK_SIZE])
        # A_block_ptr = tl.advance(A_block_ptr, [H_BLOCK_SIZE, 0])

    # x_grad_block_ptr = tl.advance(x_grad_block_ptr, [0, -H])
...
class LinearCrossEntropyLoss(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device(x.device.index):  # actually required
            ...
            # linear_xent_bwd_kernel_matmul_t_dA_dx[grid](
            #     x,
            #     y,
            #     At,
            #     lse_global,
...
if __name__ == "__main__":
    ...
    simple_bench(lambda: compiled_torch(x.float(), y, A.float()), reference_loss, reference_x_grad, reference_A_grad)
