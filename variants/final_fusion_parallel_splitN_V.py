...
def early_config_prune(configs, named_args, **kwargs):
    ...
    device = named_args["x_ptr"].device.index
    dtsize = named_args["x_ptr"].element_size()
    ...
    #         (config.kwargs["V_BLOCK_SIZE"] + config.kwargs["N_BLOCK_SIZE"])
    #         * config.kwargs["H_BLOCK_SIZE"]
    #         * config.num_stages
    #         * dtsize
    #     )
    #     breakpoint()
...
if __name__ == "__main__":
    ...
    [print(c) for c in set(linear_cross_entropy.chosen_fwd_configs)]
    [print(c) for c in set(linear_cross_entropy.chosen_bwd_configs)]
