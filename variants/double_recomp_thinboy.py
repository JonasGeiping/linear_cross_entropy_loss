...
@triton.autotune(
    configs=[
        # triton.Config({"V_BLOCK_SIZE": 16, "N_BLOCK_SIZE": 8}),
        # triton.Config({"V_BLOCK_SIZE": 32, "N_BLOCK_SIZE": 8}),
        # triton.Config({"V_BLOCK_SIZE": 64, "N_BLOCK_SIZE": 8}),
        # triton.Config({"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 8}),
        triton.Config({"V_BLOCK_SIZE": 16, "N_BLOCK_SIZE": 4}),
        # triton.Config({"V_BLOCK_SIZE": 32, "N_BLOCK_SIZE": 4}),
        # triton.Config({"V_BLOCK_SIZE": 64, "N_BLOCK_SIZE": 4}),
        # triton.Config({"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 4}),
        # triton.Config({"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 4}),
        # triton.Config({"V_BLOCK_SIZE": 16, "N_BLOCK_SIZE": 2}),
        # triton.Config({"V_BLOCK_SIZE": 32, "N_BLOCK_SIZE": 2}),
        # triton.Config({"V_BLOCK_SIZE": 64, "N_BLOCK_SIZE": 2}),
        # triton.Config({"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 2}),
        # triton.Config({"V_BLOCK_SIZE": 256, "N_BLOCK_SIZE": 2}),
    ],
    key=["V", "N", "H"],
...
def linear_xent_bwd_kernel_matmul_t_dx(
    ...
    for idx_V in range(V // V_BLOCK_SIZE):
        ...
        #     order=(1, 0),
        # )
        # A_v_full = tl.load(A_slice_ptr).trans()  # H x Vc
        # xgrad
        ...
        # mask_h = (tl.arange(0, H // H_BLOCK_SIZE) == idx_H)[None, None, :]
        x_grad_slice += temp_xgrad  # temp_xgrad tl.where(mask_h, temp_xgrad, 0.0)
