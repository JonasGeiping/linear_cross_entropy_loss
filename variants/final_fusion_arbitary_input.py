...
def linear_xent_fwd_kernel_matmul_t(
    ...
    for _ in range(H // H_BLOCK_SIZE):
        x_chunk = tl.load(x_block_ptr, boundary_check=(0, 1), padding_option="zero")  # Nc x H
        A_v = tl.load(A_block_ptr, boundary_check=(0, 1), padding_option="zero")  # Vc x H
...
def linear_xent_bwd_kernel_matmul_t_epilogue_dx(
    ...
    num_idx_N, num_idx_H = tl.num_programs(0) - (triton.cdiv(V, V_BLOCK_SIZE) * SPLIT_N), tl.num_programs(1)  # type: ignore
...
def logsumexp_reduction_kernel(
    ...
    V: tl.constexpr,
    V_BLOCK_SIZE: tl.constexpr,
    N_BLOCK_SIZE: tl.constexpr = 32,  # type: ignore
):
    idx_N = tl.program_id(axis=0)
    lse_row_ptr = tl.make_block_ptr(
        base=lse_local_ptr,
