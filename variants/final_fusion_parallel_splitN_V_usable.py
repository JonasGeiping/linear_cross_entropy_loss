...
def early_config_prune(configs, named_args, **kwargs):
    ...
    if dtype not in [torch.float16, torch.float32]:
        configs = [
            config
            for config in configs
            if (config.kwargs.get("SPLIT_N", 1) == 1) or (config.kwargs.get("SPLIT_V", 1) == 1)
        ]
    # print(len(configs))
    if len(configs) == 0:
...
def linear_xent_fwd_kernel_matmul_t(
    ...
    # save z for later
    tl.store(z_block_ptr, (z_j_to_k + tl.log(1 / reduction)).to(z_nv_ptr.type.element_ty))  # can move +log(1/N) here
...
def linear_xent_bwd_kernel_matmul_t_epilogue_dx(
    ...
    for _ in range(0, tl.cdiv(V, V_BLOCK_SIZE * SPLIT_V)):
        ...
        z_grad = softmax_z - tl.where(mask, 1 / reduction, 0.0)  # 1/N, 0 if log(1/N) moved
...
bwd_configs = [
    ...
    #     num_warps=8,
    #     num_stages=3,
    # ),  # does not fit anymore on latest triton
    triton.Config(
        {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 128, "GROUP_SIZE": 32, "SPLIT_N": 1, "SPLIT_V": 4},
        num_warps=8,
        # num_stages=2,
    ),
    triton.Config(
        {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 128, "GROUP_SIZE": 16, "SPLIT_N": 1, "SPLIT_V": 1},
        num_warps=8,
        # num_stages=2,
    ),
    # triton.Config(
    ...
    # ), # does not fit anymore on latest triton
    triton.Config(
        {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 128, "GROUP_SIZE": 32, "SPLIT_N": 1, "SPLIT_V": 1},
        num_warps=8,
        # num_stages=2,
    ),
    triton.Config(
    ...
    triton.Config(
        {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 128, "GROUP_SIZE": 16, "SPLIT_N": 1, "SPLIT_V": 2},
        num_warps=8,
        # num_stages=2,
    ),
    triton.Config(
        {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 64, "H_BLOCK_SIZE": 128, "GROUP_SIZE": 16, "SPLIT_N": 1, "SPLIT_V": 4},
        num_warps=8,
        # num_stages=2,
    ),
    # triton.Config(
    #     {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 128, "H_BLOCK_SIZE": 128, "GROUP_SIZE": 64, "SPLIT_N": 1, "SPLIT_V": 4},
    #     num_warps=16,
    #     # num_stages=2,
    # ),
    # triton.Config(
    #     {"V_BLOCK_SIZE": 128, "N_BLOCK_SIZE": 128, "H_BLOCK_SIZE": 128, "GROUP_SIZE": 64, "SPLIT_N": 1, "SPLIT_V": 8},
    #     num_warps=16,
    #     # num_stages=2,
    # ),
    # triton.Config(
