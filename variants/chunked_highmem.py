...
class LinearCrossEntropyLoss(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device(x.device.index):  # actually required
            ...
            V_tile_size = min(H, V)
            N_tile_size = min(H, N)  # max memory peak is now H**3
            for v_idx in range(V // V_tile_size):
                for n_idx in range(N // N_tile_size):
                    ...
                    lse_tile = lse_global[n_idx * N_tile_size : (n_idx + 1) * N_tile_size]
                    sz = torch.empty((N_tile_size, V_tile_size), dtype=torch.float16, device=x.device)
                    ...
                    xgrad[n_idx * N_tile_size : (n_idx + 1) * N_tile_size, :] += x_grad_tile
                    Atgrad[:, v_idx * V_tile_size : (v_idx + 1) * V_tile_size] += At_grad_tile
